...
export function OrderDetailContent({ orderId, basePath }: OrderDetailContentProps) {
  const router = useRouter()
  const queryClient = useQueryClient()

  // Use the admin service for fetching fully detailed orders.
  // Assuming the role guard protects the page and staff can use the same API if allowed.
  const { data: orderData, isLoading } = useQuery({
    queryKey: ['order', orderId, basePath],
  ...
  const updateStatusMutation = useMutation({
    ...
    onSuccess: (_, variables) => {
      ...
      queryClient.setQueryData(['order', orderId, basePath], (old: any) => {
  ...
  const { data: shippersData, isLoading: isLoadingShippers } = useQuery({
    ...
    enabled:
      ((localStatus || order?.status) === 'CONFIRMED' && !isAssigned) || basePath === 'admin',
  })
      ...
      <div className='flex flex-col md:flex-row md:items-center justify-between gap-4'>
        ...
        <div className='flex flex-wrap sm:flex-nowrap items-center gap-3 w-full md:w-auto'>
          <Button
            ...
            onClick={() => window.open(`/${basePath}/orders/${orderId}/print`, '_blank')}
          >
